...
public class Proxy {
    ...
    private Response redirect(UriInfo uri, HttpServletRequest request) {
        ...
        // If the request has the 'X-Requires-Gateway-Auth' header set to 'true' by the client,
        // then append a special auth token generated by the gateway to the redirect url's query string
        String requiresGatewayHeader = request.getHeader("X-Requires-Gateway-Auth");
        if (requiresGatewayHeader != null && requiresGatewayHeader.equals("true")) {
            String gatewayToken = JWTHandler.createGatewaySharedToken();
            redirectUrl = UriBuilder
                    .fromUri(redirectUrl)
                    .queryParam("gateway_token", gatewayToken)
                    .build()
                    .toString();
        }
